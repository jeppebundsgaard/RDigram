...
proficiency.scale<-function(mod=NULL,dontplot=F,font.base.size = 10 / .pt,
                            ...
                            level.name.family="serif",level.name.size=font.base.size*1.1,level.name.fontface="bold",level.name.color="white",level.name.hjust="left",level.name.nudge_x=.02,level.name.nudge_y=.1,
  ...
  if(!is.null(items.include.categories)) {
    item.params<-item.params[,1:items.include.categories]
  ...
  data<-data.frame(level.names,level.descriptions)
  level.rect.left<-0
  ...
  pl<-
    ...
    # Header
    geom_tile(aes(x = level.rect.left+level.rect.width/2, width = level.rect.width, y = level.lims[nlevels+1]+header.heigth/2, height = header.heigth),
              color = header.rect.color,  fill = header.rect.fill) +
    geom_text(aes(x = level.rect.left+level.rect.width/2, y = level.lims[nlevels+1]+header.heigth,label = header.level),
    ...
    geom_tile(aes(x = items.rect.left+items.rect.width/2, width = items.rect.width, y = level.lims[nlevels+1]+header.heigth/2, height = header.heigth),
              color = header.rect.color,  fill = header.rect.fill) +
    geom_text(aes(x = items.rect.left+items.rect.width/2, y = level.lims[nlevels+1]+header.heigth,label = header.item),
              family=header.family,
              nudge_x = header.nudge_x,nudge_y = -header.nudge_y,
              hjust = header.hjust, vjust = "top",
              size = header.size, fontface=header.fontface,color=header.color) +
    ...
    # Level descriptions
    geom_rect(aes(xmin = level.rect.left, xmax = level.rect.right, ymin = level.lims[1:nlevels], ymax = level.lims[2:(nlevels+1)]),
              color = level.rect.color,  fill = level.rect.fill) +
    geom_text(aes(x = level.rect.left, y = level.lims[2:(nlevels+1)],
                label = level.names),
      ...
      geom_text(aes(x = level.rect.left , y = level.lims[2:(nlevels+1)],
                    label = linebreak(level.descriptions,len = level.description.linelength)),
                family=level.description.family,
                nudge_x = level.description.nudge_x,nudge_y = -level.description.nudge_y,
      ...
      # Item descriptions
      geom_rect(aes(xmin = items.rect.left, xmax = items.rect.right, ymin = level.lims[1:nlevels], ymax = level.lims[2:(nlevels+1)]),
                color = items.rect.color, fill = items.rect.fill) +
      geom_text(aes(x = items.rect.left , y = level.lims[2:(nlevels+1)],
  ...
  # Correct response probabilities
      if(person.dist.type=="in.levels"){
        ...
        if(header.show.percentage.in.group && !is.null(mod)) {
          grps<-table(cut(mod$person$EAP,breaks = person.lims))
          header.persons<-sapply(1:ngroups,function(g) paste0(header.persons[g],"\n(",format(grps[g]/mod$nstud*100,digits = 0)," %)"))
        }
        for(g in 1:ngroups) {
          ps<-virtual.p[[g]]$p[,2]
          probs<-round(ps*100)
          ...
          pl<-pl+
            geom_tile(aes(x = person.rect.left[!!g]+person.rect.width/2, width = person.rect.width, y = level.lims[nlevels+1]+header.heigth/2, height = header.heigth),
                      color = header.rect.color,  fill = header.rect.fill) +
            geom_text(aes(x = person.rect.left[!!g]+person.rect.width/2, y = level.lims[nlevels+1]+header.heigth,label = header.persons[!!g]),
                      family=header.family,
                      nudge_x = header.nudge_x,nudge_y = -header.nudge_y,
                      hjust = header.hjust, vjust = "top",
                      size = header.size, fontface=header.fontface,color=header.color) +
            ...
            geom_tile(data=data.frame(fill=seq(higher,lower,length.out = 100)),aes(x = person.rect.left[!!g]+person.rect.width/2, width = person.rect.width, y = seq(level.lims[1]+yby/2,level.lims[nlevels+1]-yby/2,length.out = 100), fill = fill)#level.lims[1:ngroups], ymax = level.lims[2:(ngroups+1)]
                    ) +
            geom_rect(aes(xmin = person.rect.left[!!g], xmax = person.rect.right[!!g], ymin = level.lims[1],ymax=level.lims[nlevels+1]),color = person.rect.color,fill=NA)+#,
            geom_text(data=data.frame(1:(nlevels+1)),aes(x = person.rect.left[!!g]+person.rect.width/2 , y = level.lims[1:(nlevels+1)],
                      label = !!problims),
